...
{\Huge\scshape
Bro\hspace{-2pt}\raisebox{15pt}{\tiny{master}}\hspace{-4pt}
Cheat Sheet}
...
\begin{tabular}{l l}
Version: & \today\\
Website: & \url{http://www.bro.org}\\
Email: & \texttt{\href{mailto:info@bro.org}{info@bro.org}}\\
Download: & \url{https://github.com/bro/cheat-sheet}\\
...
\verb#|x|# \dotfill Cardinality/size for addresses, strings, containers\\
...
\minisec{Basic}
\verb|addr| \dotfill IP address (v4: \verb|127.0.0.1|, v6: \verb|[fe80::db15]|)\\
...
% TODO. Not yet supported in Bro, see #679.
...
\verbose{
  \item \verb|system_env(s: string, env: table[string] of string): int|\\
  ...
  \item \verb|md5_hash_init(): opaque of md5|\\
    Retrieves an opaque handle for incremental MD5 hash computation. Add data
    with via \verb|md5_hash_update|.
  \item \verb|md5_hash_update(handle: opaque of md5, data: string): bool|\\
    Feeds an incremental MD5 computation with \verb|data|. Call
    \verb|md5_hash_finish(handle)| to retrieve the final hash digest.
  \item \verb|md5_hash_finish(handle: opaque of md5): string|\\
    Returns the final MD5 digest of \verb|handle|. This invalidates
    \verb|handle|, i.e., the function can only be called once.
  ...
  \item \verb|sha1_hash(...): string|\\
    Computes the SHA1 hash value of the provided list of arguments.
    Analogous to \verb|md5_hash|.
  \item \verb|sha1_hash(...): string|\\
    Computes the SHA1 hash value of the provided list of arguments.
  \item \verb|sha1_hash_init(): opaque of sha1|\\
    Retrieves an opaque handle for incremental SHA1 hash computation. Add data
    with via \verb|sha1_hash_update|.
  \item \verb|sha1_hash_update(handle: opaque of sha1, data: string): bool|\\
    Feeds an incremental SHA1 computation with \verb|data|. Call
    \verb|sha1_hash_finish(handle)| to retrieve the final hash digest.
  \item \verb|sha1_hash_finish(handle: opaque of sha1): string|\\
    Returns the final SHA1 digest of \verb|handle|. This invalidates
    \verb|handle|, i.e., the function can only be called once.
  \item \verb|sha256_hash(...): string|\\
    Computes the SHA256 hash value of the provided list of arguments.
    Analogous to \verb|md5_hash|.
  \item \verb|sha256_hash(...): string|\\
    Computes the SHA256 hash value of the provided list of arguments.
  \item \verb|sha256_hash_init(): opaque of sha256|\\
    Retrieves an opaque handle for incremental SHA256 hash computation. Add data
    with via \verb|sha256_hash_update|.
  \item \verb|sha256_hash_update(handle: opaque of sha256, data: string): bool|\\
    Feeds an incremental SHA256 computation with \verb|data|. Call
    \verb|sha256_hash_finish(handle)| to retrieve the final hash digest.
  \item \verb|sha256_hash_finish(handle: opaque of sha256): string|\\
    Returns the final SHA256 digest of \verb|handle|. This invalidates
    \verb|handle|, i.e., the function can only be called once.
...
\begin{itemize}
  ...
  \item \verb|capture_state_updates(filename: string): bool|\\
    ...
    file \verb|filename|.
  \item \begin{verbatim}
connect(ip: addr, zone_id: string, p: port, our_class: string, retry: interval,
        ssl: bool): count
\end{verbatim}
    ...
    connection if they don't match. If \verb|ip| is non-global IPv6 address, an
    RFC 4007 \verb|zone_id| can be given here.
  ...
  \item \begin{verbatim}
listen(ip: addr, p: port, ssl: bool, ipv6: bool, zone_id: string,
       retry: interval): bool
\end{verbatim}
    Listens on address \verb|ip| and port \verb|p| for remote connections,
    retrying after \verb|retry| if \verb|ip| is found to be already in use. If
    \verb|ssl| is true, Bro uses SSL to encrypt the session. If \verb|ipv6| is
    \verb|true|, also listen on IPv6 addresses with an optional RFC 4007
    \verb|zone_id|.
    \ReturnsTrueOnSuccess
...
\verbose{
  ...
  \item \verb|entropy_test_init(): opaque of entropy|\\
    Retrieves a data structures for incremental entropy calculation.
    \ReturnsTrueOnSuccess
    See \verb|entropy_test_add| and \verb|entropy_test_finish|.
  \item \verb|entropy_test_add(handle: opaque of entropy, data: string): bool|\\
    Adds \verb|data| to the incremental entropy calculation identified by
    \verb|handle|.
    \ReturnsTrueOnSuccess
  \item \verb|entropy_test_finish(handle: opaque of entropy): entropy_test_result|\\
    Finalizes the incremental entropy calculation identified by \verb|handle|.
\verbose{
...
\begin{itemize}
  \item \verb|is_v4_addr(a: addr): bool|\\
    Checks whether an address is IPv4. Returns \verb|true| for IPv4 and
    \verb|false| for IPv6 addresses.
  \item \verb|is_v6_addr(a: addr): bool|\\
    Checks whether an address is IPv6. Returns the opposite of
    \verb|is_v4_addr|.
...
\verbose{
  ...
  \item \verb|to_double(s: string): double|\\
    Converts a \texttt{string} into a \texttt{double}.
  ...
  \item \verb|addr_to_counts(a: addr): vector of count|\\
    Converts an IP address into a vector of of \verb|count|s in host
    byte-order. Returns 4 elements for IPv6 and one for IPv4 addresses.
  \item \verb|counts_to_addr(v: vector of count): addr|\\
    The dual to \verb|addr_to_counts|: converts a vector of counts to and IP
    address.
  \item \verb|to_subnet(ip: string): subnet|\\
    Converts a \verb|string| into a subnet type. Returns \verb|../0| if the
    input does not parse correctly.
\verbose{
  \item \verb|routing0_data_to_addrs(s: string): vector of address|\\
    Converts the \verb|data| field of \verb|ip6_routing| records (\verb|s|)
    that have \verb|rtype| of 0 into a vector of addresses.
...
\verbose{
  \item \verb|file_mode(mode: count): string|\\
