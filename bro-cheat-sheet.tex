...
\verbose{
  ...
  \item \verb|exit()|
    % TODO: Update this function signature to
    %
    %   exit(code: int)
    %
    % when the branch topic/bif_cleanup is merged into master.
...
\begin{itemize}
  ...
  \item \verb|mkdir(f: string): bool|\\
    ...
    operation succeeds and false if direction creation files or \verb|f| exists
    already.
...
\verbose{
    % TODO: Remove prefix if the function name has changed.
